            ...
            <div class="position-relative">
                ...
                <div class="login-form">
                    ...
                    <div class="form-group">
                        <input type="text"  id="exampleInputName" placeholder="Enter name" v-model="name">
                    </div>

                    <div class="form-group">
                        <input type="email"  id="exampleInputEmail1" placeholder="Enter email" v-model="email">
                    </div>

                    <div class="form-group mb-4">
                        <input type="password" id="exampleInputPassword1" placeholder="Enter Password" v-model="password">
                    </div>

                    <div class="form-group clearfix">
                        <!--<router-link to="/forgot-password"><a href="#" class="float-left forgot-link my-2">Forgot Password ?</a></router-link>-->
                        <button type="submit" class="btn-red" @click="login_button_click">Signup</button>
                    </div>
                    ...
                    <div class="text-center mt-5 mb-5">
                        <router-link class="btn-link text-capitalize f12" to="/login">Login instead</router-link>
                    </div>
                    <!--        </form>-->
                </div>
                <!--/login form-->
    ...
    login_button_click () {
      ...
      this.$auth.register({
        ...
        success: function (res) {
          app.changeUser(res.data.user)
        },
        error: function (resp) {
          console.debug(resp.response.data)
          app.makeToast(variables.danger, resp.response.data.message, 'Registration error')
        },
        redirect: '/',
        autoLogin: true,
        rememberMe: true
